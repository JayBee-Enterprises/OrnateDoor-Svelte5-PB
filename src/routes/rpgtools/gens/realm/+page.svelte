<script>
	...
	const realmStats = $derived({
		areaString: results.realmPopulation > 0 ? inputs.realmArea.toLocaleString() : '',
        ...
        otherPopulation: Math.max(0, results.realmPopulation - results.cityPopulationTotal - results.townPopulationTotal),
        get otherPopulationString() { return this.otherPopulation.toLocaleString(); }
	});

    const isFormIncomplete = $derived(
        inputs.developmentFactor <= 0 || inputs.realmAge <= 0 || inputs.realmArea <= 0
    );

    // --- Helper Functions ---
    // calculateBaseStats, generateCityData, generateTownData remain the same as the previous version
    // (including the individual caps within those functions)

    function calculateBaseStats() {
        const popDensityRoll = Dice.multipleDice(6, 4, 0);
        results.popDensity = popDensityRoll * inputs.developmentFactor;
        results.realmPopulation = Math.max(1, Math.floor(results.popDensity * inputs.realmArea));
        ...
        results.ruinsCount = results.realmPopulation > 0 && RUINS_POPULATION_FACTOR > 0
            ? Math.floor((results.realmPopulation / RUINS_POPULATION_FACTOR) * ageSqrt)
            : 0;
    }

    function generateCityData() {
        results.cityPopArray.length = 0;
        if (results.realmPopulation <= 0) { /* ... rest of empty checks ... */ return; }

        const p = Math.sqrt(results.realmPopulation);
        const m = Dice.multipleDice(LARGEST_CITY_DICE_COUNT, LARGEST_CITY_DICE_SIDES, LARGEST_CITY_MODIFIER);
        results.largestCityPop = Math.floor(p * m);
        results.largestCityPop = Math.min(results.largestCityPop, results.realmPopulation);
        results.largestCityPop = Math.max(0, results.largestCityPop);

        const secondCityRoll = Dice.multipleDice(SECOND_LARGEST_CITY_DICE_COUNT, SECOND_LARGEST_CITY_DICE_SIDES, 0);
        results.secondLargestCityPop = Math.floor(results.largestCityPop * (secondCityRoll * 0.1));
        results.secondLargestCityPop = Math.min(results.secondLargestCityPop, results.realmPopulation);
        results.secondLargestCityPop = Math.max(0, results.secondLargestCityPop);

        if (results.largestCityPop >= CITY_POPULATION_THRESHOLD) { results.cityPopArray.push(results.largestCityPop); }
        if (results.secondLargestCityPop >= CITY_POPULATION_THRESHOLD && results.secondLargestCityPop < results.largestCityPop) {
             if (!results.cityPopArray.includes(results.secondLargestCityPop)) { results.cityPopArray.push(results.secondLargestCityPop); }
        }

        let currentCityPop = results.secondLargestCityPop;
        let iterations = 0;
        const maxIterations = 1000;
        while (currentCityPop >= CITY_POPULATION_THRESHOLD && iterations < maxIterations) {
            iterations++;
            const randomRoll = Dice.multipleDice(NEXT_CITY_DICE_COUNT, NEXT_CITY_DICE_SIDES, 0);
            const lastPop = results.cityPopArray.length > 0 ? results.cityPopArray[results.cityPopArray.length - 1] : 0;
            currentCityPop = Math.floor(lastPop * (1 - randomRoll * NEXT_CITY_REDUCTION_FACTOR));
            currentCityPop = Math.min(currentCityPop, results.realmPopulation);
            currentCityPop = Math.max(0, currentCityPop);

            if (currentCityPop >= CITY_POPULATION_THRESHOLD) {
                 if (results.cityPopArray.length === 0 || results.cityPopArray[results.cityPopArray.length - 1] !== currentCityPop) {
                    results.cityPopArray.push(currentCityPop);
                 } else { break; }
            } else { break; }
        }
         if (iterations >= maxIterations) { console.warn("generateCityData: Max iterations reached"); }
     ...
     function generateTownData() {
        results.townPopArray.length = 0;
        if (results.realmPopulation <= 0) { /* ... rest of empty checks ... */ return; }

        const baseTownCount = results.cityCount > 0
            ? results.cityCount * Dice.multipleDice(TOWN_COUNT_DICE_COUNT, TOWN_COUNT_DICE_SIDES, 0)
            : Math.max(1, Math.floor(Math.sqrt(results.realmPopulation) / 50));
        results.townCount = Math.max(0, baseTownCount);

        for (let i = 0; i < results.townCount; i++) {
            let townPopulation;
            const randomRoll = Math.random();
            if (randomRoll > TOWN_POP_HIGH_ROLL_THRESHOLD) {
                townPopulation = Math.floor(weightedRandom(TOWN_POP_HIGH_WEIGHT_MAX, TOWN_POP_HIGH_WEIGHT_FACTOR));
            } else {
                townPopulation = Math.floor(weightedRandom(TOWN_POP_LOW_WEIGHT_MAX, TOWN_POP_LOW_WEIGHT_FACTOR));
            }
            if (townPopulation < MIN_TOWN_POPULATION_BASE) { townPopulation += MIN_TOWN_POPULATION_BASE; }
            if(townPopulation >= CITY_POPULATION_THRESHOLD) { townPopulation = CITY_POPULATION_THRESHOLD - Dice.multipleDice(1, 100, 1); }
            townPopulation = Math.max(0, townPopulation);
            townPopulation = Math.min(townPopulation, results.realmPopulation);

            if (townPopulation >= MIN_TOWN_POPULATION_BASE) { // Only add if above minimum threshold *after* all adjustments
                results.townPopArray.push(townPopulation);
            }
        }
        results.townCount = results.townPopArray.length; // Update count based on valid towns added
        results.townPopArray.sort((a, b) => b - a);
	...
	function realmGenerator() {
        ...
        inputs.developmentFactor = Number(inputs.developmentFactor) || 0;
        inputs.realmAge = Number(inputs.realmAge) || 0;
        inputs.realmArea = Number(inputs.realmArea) || 0;
        ...
        // *** ADDED: Population Sum Constraint Check and Adjustment ***
        const initialTotalSettlementPop = results.cityPopulationTotal + results.townPopulationTotal;

        if (initialTotalSettlementPop > results.realmPopulation && initialTotalSettlementPop > 0) {
            // Calculate scaling factor only if total pop is positive to avoid division by zero
            const scaleFactor = results.realmPopulation / initialTotalSettlementPop;

            // Scale and Filter Cities
            results.cityPopArray = results.cityPopArray
                .map(pop => Math.floor(pop * scaleFactor)) // Scale down
                .filter(pop => pop >= CITY_POPULATION_THRESHOLD); // Filter out those below threshold

            // Scale and Filter Towns
            results.townPopArray = results.townPopArray
                .map(pop => Math.floor(pop * scaleFactor)) // Scale down
                .filter(pop => pop >= MIN_TOWN_POPULATION_BASE); // Filter out those below threshold

            // Recalculate totals and counts AFTER scaling and filtering
            results.cityPopulationTotal = results.cityPopArray.reduce((a, b) => a + b, 0);
            results.townPopulationTotal = results.townPopArray.reduce((a, b) => a + b, 0);
            results.cityCount = results.cityPopArray.length;
            results.townCount = results.townPopArray.length;

             // Ensure town populations are still sorted after potential removals/scaling
             results.townPopArray.sort((a, b) => b - a);
        }
        // *** End of Adjustment ***
	...
	function resetForm() {
        ...
        results.cityPopArray = [];
        results.townPopArray = [];
        results.cityCount = 0;
        results.townCount = 0;
        ...
        <form onsubmit={preventDefault(realmGenerator)}>
			<label class="form-control w-full max-w-lg pt-4">
				...
				<select bind:value={inputs.developmentFactor} class="select select-bordered select-primary w-full" required>
					<option disabled value={0}>Pick one</option> <!-- Use value=0 for default/disabled -->
			...
			<div class="w-full pt-4">
                <button type="submit" class="btn btn-primary mx-auto w-full" disabled={isFormIncomplete}>
                    Generate Realm
                </button>
			</div>
		</form>
...
{#if !showForm}
	<div class="grid md:grid-cols-2 gap-4 p-4" id="realmGenResults">
        ...
        <div class="col-span-full mt-6 flex justify-around">
            <form onsubmit={preventDefault(resetForm)} class="w-full max-w-xs">
				<button type="submit" class="btn btn-primary mx-auto w-full">Generate New Realm</button>
			</form>
			<form onsubmit={preventDefault(realmGenerator)} class="w-full max-w-xs">
				<button type="submit" class="btn btn-accent mx-auto w-full">Re-Generate Realm</button>
			</form>
		</div>
	</div>
{/if}